...
def get_a_word():
    url = f"https://api.datamuse.com/words?sp={'?'*NUM_LETTERS}&md=f&max=1000"
...
|   :    /  |  | ,'        ;   | |`-'      |   :   .'           \   \ .'        :   \ ||   :   .':   : :-'   
 \   \ .'   `--''          |   ;/          |   | ,'              `---`           '---" |   | ,'  |   |.'     
  `---`                    '---'           `----'                                      `----'    `---'                                                                                                
""")
print(f"SCORE: {round((DIFFICULTY/score)*1000)} {DEFAULT_COLOR}")

if guess != the_word:
    input("Click enter to find out the word...")
    print_word(the_word, [GREEN for _ in the_word])
